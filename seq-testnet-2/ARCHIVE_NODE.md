...
This guide covers how to run an archive node for `seq-testnet-2` and how to migrate archive data between hosts.
...
tar -czf "/tmp/seq-testnet-2-archive-data-$(date +%F).tar.gz" -C "$HOME" .fuelsequencer/data
shasum -a 256 "/tmp/seq-testnet-2-archive-data-$(date +%F).tar.gz" > "/tmp/seq-testnet-2-archive-data-$(date +%F).sha256"
```
...
```sh
systemctl stop fuelsequencer
shasum -a 256 -c /tmp/seq-testnet-2-archive-data-YYYY-MM-DD.sha256
rm -rf "$HOME/.fuelsequencer/data"
tar -xzf /tmp/seq-testnet-2-archive-data-YYYY-MM-DD.tar.gz -C "$HOME"
